...
public class ViewProfileController implements UserInterface,
    ...
    private void initialize() {
        ClientConnection conn = ClientConnection.getInstance();
        conn.registerFriendListener(this);
        conn.setCurrentController(this);

        backBtn.setOnAction(e ->
        {
            conn.removeFriendListener(this);
            sceneChange.changeScene("Dashboard.fxml", backBtn, currentUser);
        });
    }
    ...
    private void loadImage() {
        File imgFile = new File("users/" + targetUser.getUsername() + "/profile.jpg");
        Image image;
        ...
        double radius = Math.min(profilePic.getFitWidth(), profilePic.getFitHeight()) / 2;
        Circle clip = new Circle(profilePic.getFitWidth() / 2, profilePic.getFitHeight() / 2, radius);
    ...
    public void onFriendRequestReceived(String from) {
        Platform.runLater(this::refreshProfile);
    }
